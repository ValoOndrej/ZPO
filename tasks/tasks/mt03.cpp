...
int passFilter( const cv::Mat& src, cv::Mat& dst, int limit_frequency, int flag, cv::Mat * spektrum = NULL )
{
    ...
    /*  Working area - begin */
	int cx = spectrumCplx.rows/2.0;
	int cy = spectrumCplx.cols/2.0;

	int range = limit_frequency/2.0;
	...
	if( flag == HIGH_PASS_FILTER ) {
        ...
        for (int i=cx-range; i < cx+range; ++i){
            for (int j=cy-range; j < cy+range; ++j){
                spectrumCplx.at<double>(i,j) = int(0);
            }
        }
	}
        ...
        for (int i=0; i < spectrumCplx.rows; ++i){
            for (int j=0; j < spectrumCplx.cols; ++j){
                if (i >= cx+range or i < cx-range or j >= cy+range or j < cy-range){
                    spectrumCplx.at<double>(i,j) = int(0);
                }
            }
        }
